    ...
    while True:
        ...
        try:
            if value_entered in ['chicago', 'new york city', 'washington'] and value_int==0:
                ...
                if value_int==0:
                    print('wrong city entered.Please enter a valid city from the given list')
                if value_int==1:
                    print('wrong month entered.Please enter a valid month from the given list')
                if value_int==2:
...
def get_filters():
    ...
    city = check_user_value('Enter a city from the given list: chicago, new york city or washington\n: ', 0)


    # TO DO: get user input for month (all, january, february, ... , june)
    month= check_user_value("Enter a month from - (all, january, february, ... , june): ", 1)

    # TO DO: get user input for day of week (all, monday, tuesday, ... sunday)
    day= check_user_value("which day of the week?(all, monday, tuesday, ... sunday): ", 2)
...
def station_stats(df):
    ...
    print("The most frequent combination of start station and end station trip is: {}".format(df['frequent_route'].mode()[0]))
...
def user_stats(df,city):
    ...
    if city != 'washington':
        print("Total counts of gender is: ",(df['Gender'].value_counts().to_frame()))
        ...
        print("the most occuring year of bith is: ",int(df['Birth Year'].mode()[0]))
    else:
...
def data_display(df):
    ...
    #get user input
    user_input = input("Would like to display the data in rows of 5?, Please enter yes or no \n: ")
    if user_input.lower() == 'yes':
        value=0
        while True:
            print(df.iloc[value:value+5])
            value += 5
            next_input = input("Would you like to display more 5 rows of data?, yes or no\n: ")
